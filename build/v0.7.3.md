...
- **Multi-Shell Support**: Detects and updates the appropriate profile file:
  - Priority 1: `~/.bash_profile` (preferred for Bash - required for macOS login shells)
  - Priority 2: `~/.profile` (POSIX-compliant shells)
...
### Profile Detection for macOS

- **Fixed**: Profile detection now always uses `~/.bash_profile` instead of `~/.bashrc` on macOS
  - Issue: `.bashrc` is not loaded by login shells on macOS, preventing automatic environment loading
  - Solution: Removed `.bashrc` from priority list; now uses `.bash_profile` (Priority 1), `.profile` (Priority 2), or `.zshrc` (Priority 3)
  - Impact: Login shells on macOS will now correctly load the Oracle environment
...
✅ **Be flexible for profile default bash but add corresponding fallbacks**
   - Supports 3 shell profiles with priority order
   - bash_profile → profile → zshrc
   - Creates bash_profile if none exist
   - **Fixed**: Removed bashrc as it doesn't work for login shells on macOS
