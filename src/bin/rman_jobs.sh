...
debug_log "Executing: ${SCRIPT_DIR}/longops.sh --operation RMAN% ${filtered_args[*]}"

# Execute longops.sh with RMAN filter
if [[ "${DEBUG_ENABLED}" == "true" ]]; then
    # Pass debug flag to longops.sh if debug is enabled
    exec "${SCRIPT_DIR}/longops.sh" --operation "RMAN%" --debug "${filtered_args[@]}"
else
    exec "${SCRIPT_DIR}/longops.sh" --operation "RMAN%" "${filtered_args[@]}"
fi
