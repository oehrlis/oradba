...
check_debug_activation() {
    ...
    if [[ "${DEBUG_ENABLED}" == "true" ]]; then
        ...
        debug_log "Starting DataPump import monitoring wrapper script"
        debug_log "Script directory: ${SCRIPT_DIR}"
        debug_log "Target script: ${SCRIPT_DIR}/longops.sh"
        debug_log "Operation filter: %IMP%"
...
debug_log "Executing: ${SCRIPT_DIR}/longops.sh --operation %IMP% ${filtered_args[*]}"

# Execute longops.sh with import filter
if [[ "${DEBUG_ENABLED}" == "true" ]]; then
    # Pass debug flag to longops.sh if debug is enabled
    exec "${SCRIPT_DIR}/longops.sh" --operation "%IMP%" --debug "${filtered_args[@]}"
else
    exec "${SCRIPT_DIR}/longops.sh" --operation "%IMP%" "${filtered_args[@]}"
fi
