# oradba User Documentation

This directory contains user-facing documentation that is installed with oradba.

## Available Documentation

- **[USAGE.md](USAGE.md)** - Comprehensive usage guide
- **[SCRIPTS.md](SCRIPTS.md)** - Description of all included scripts
- **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)** - Common issues and solutions
- **[EXAMPLES.md](EXAMPLES.md)** - Practical examples and use cases

## Quick Links

- Main project: <https://github.com/oehrlis/oradba>
- Issue tracker: <https://github.com/oehrlis/oradba/issues>
- Discussions: <https://github.com/oehrlis/oradba/discussions>

## Getting Help

1. Check this documentation
2. Review examples in `src/sql/` and `src/templates/`
3. Read the troubleshooting guide
4. Open an issue on GitHub

## License

Apache License 2.0 - See LICENSE file for details.
