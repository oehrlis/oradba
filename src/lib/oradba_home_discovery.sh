...
generate_oracle_home_aliases() {
    ...
    while IFS=: read -r name _path _type _order alias_name _desc _version; do
        ...
        alias "${name_lower}"=". ${ORADBA_BASE}/bin/oraenv.sh ${name}"
        ...
        if [[ -n "${alias_name}" && "${alias_name}" != "${name_lower}" ]]; then
            # shellcheck disable=SC2139
            alias "${alias_name}"=". ${ORADBA_BASE}/bin/oraenv.sh ${name}"
...
detect_oracle_version() {
    ...
    if execute_plugin_function_v2 "${product_type}" "get_version" "${oracle_home}" "plugin_version"; then
        ...
        local _maj_rest="${plugin_version#*.}"
        minor="${_maj_rest%%.*}"
...
auto_discover_oracle_homes() {
    ...
    if ! config_file=$(get_oracle_homes_path 2> /dev/null); then
        config_file="${ORADBA_BASE}/etc/oradba_homes.conf"
    fi
...
oradba_get_oracle_homes_path() { get_oracle_homes_path "$@"; }
oradba_detect_product_type() { detect_product_type "$@"; }
oradba_set_oracle_home_environment() { set_oracle_home_environment "$@"; }
