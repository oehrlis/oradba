...
echo ""; \
echo "Location: $sqlpath"; \
if [ -n "$filter" ]; then echo "Filter: *${filter}*"; fi; \
echo ""; \
printf "%-30s %s\n" "Script" "Purpose"; \
printf "%-30s %s\n" "$(printf "%.30s" "------------------------------")" "$(printf "%.60s" "------------------------------------------------------------")"; \
cd "$sqlpath" && \
for f in *.sql; do \
  if [ -f "$f" ]; then \
    if [ -z "$filter" ] || echo "$f" | grep -qi "$filter"; then \
      purpose=$(head -20 "$f" | grep -i "^--.*Purpose" | head -1 | sed "s/.*Purpose[.:]*//;s/^[[:space:]]*//;s/[[:space:]]*$//" | cut -c1-60); \
      if [ -n "$purpose" ]; then \
        printf "%-30s %s\n" "$f" "$purpose"; \
      fi; \
    fi; \
