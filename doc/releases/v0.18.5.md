...
This release addresses five configuration and environment management improvements including template organization, intelligent ORACLE_BASE derivation, Oracle version detection, and better handling of non-root environments.
...
**Implementation:**
- Added `derive_oracle_base()` function in `common.sh`
...
**Files Changed:**
- `src/lib/common.sh` - Added `derive_oracle_base()` function
...
- `src/lib/common.sh` - Added `detect_oracle_version()` and `get_oracle_home_version()` functions
- `src/lib/common.sh` - Updated parsing functions to handle VERSION field
...
- Added dynamic initialization in `oraenv.sh` after loading `common.sh`
...
#### `derive_oracle_base()` in `common.sh`
```bash
...
#### `detect_oracle_version()` in `common.sh`
```bash
...
#### `get_oracle_home_version()` in `common.sh`
```bash
...
### Scripts & Libraries
- `src/lib/common.sh` - Added version detection and ORACLE_BASE derivation functions
