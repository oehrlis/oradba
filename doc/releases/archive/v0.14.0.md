...
### 🔄 Function Rename: `log()` → `oradba_log()` (Issue #16)
...
- Deprecated wrapper functions remain for compatibility:
  - `log_info()` → calls `oradba_log INFO`
  - `log_warn()` → calls `oradba_log WARN`
  - `log_error()` → calls `oradba_log ERROR`
  - `log_debug()` → calls `oradba_log DEBUG`
...
### 🐛 SID Config Auto-Creation Fixed (Issue #16)
...
2. **Wrong regex pattern** in `common.sh`:
