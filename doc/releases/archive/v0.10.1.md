...
- Changed from outdated `sid.ORACLE_SID.conf.example` (now in `src/templates/etc/`) to current `sid.ORCL.conf.example`
...
- Library files (oradba_common.sh, oradba_aliases.sh, oradba_db_functions.sh)
