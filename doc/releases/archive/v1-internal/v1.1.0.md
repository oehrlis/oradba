...
- 🔒 Full Data Safe On-Premises Connector support (DATASAFE product type)
- ⚡ Instant Client (ICLIENT) support with proper detection and environment setup
- 🎯 Direct cmctl status checking (50-100ms faster than Python-based approach)
- 🔄 PATH deduplication to prevent repeated entries
- 📊 Improved oraup.sh display for non-database Oracle Homes
- 🔧 Parser format alignment for consistency across the codebase
...
- ✅ Direct cmctl status checking (50-100ms faster than Python)
- ✅ Proper PATH and LD_LIBRARY_PATH configuration
- ✅ Validation skips sqlplus check (not required for Data Safe)
- ✅ Live status display: RUNNING/STOPPED/UNKNOWN
- ✅ Connector name display in oraup.sh
...
- ✅ Proper PATH setup (ORACLE_HOME directly, not bin)
- ✅ Proper LD_LIBRARY_PATH setup (ORACLE_HOME directly)
- ✅ Distinct from full CLIENT product type
...
# On database/client installations → uses native tnsping
TNS Ping Utility for Linux: Version 19.0.0.0.0 - Production
Attempting to contact FREE
OK (10 msec)

# On Instant Client → automatically uses sqlplus -P
...
- ✅ Automatic tool detection (tnsping or sqlplus -P)
- ✅ Works with TNS names and EZ Connect strings
- ✅ Clear error messages for unsupported formats
- ✅ Informational notice in verbose/debug mode
- ✅ Preserves all command-line arguments
- ✅ Only active when ORACLE_HOME is set
...
Prevents PATH pollution from repeated JDK paths and other duplicate directories that
accumulate when sourcing environment multiple times or loading custom configurations.
...
# - Final deduplication after custom config loading
```
...
- ✅ Removes duplicate LD_LIBRARY_PATH entries
- ✅ Preserves PATH order
- ✅ Platform-aware (handles SHLIB_PATH, LIBPATH, DYLD_LIBRARY_PATH)
- ✅ Final cleanup after all custom configs loaded (JAVA_HOME, etc.)
- ✅ Consolidated implementation - single source of truth

**New Functions:**

- `oradba_dedupe_path()` - Core PATH deduplication logic
- `deduplicate_path()` - Wrapper using oradba_dedupe_path()
- `deduplicate_sqlpath()` - SQLPATH deduplication

**Technical Details:**

The final deduplication step runs in `set_oracle_home_environment()` after
`oradba_apply_product_config()` completes. This ensures custom PATH additions
from `oradba_customer.conf` or SID-specific configs (like JAVA_HOME, MAVEN_HOME)
are properly deduplicated.

### Dummy Entry Configuration

**Feature:** Configurable dummy entry visibility

Provides control over dummy database entry display in environments that will never
have Oracle Database installations (e.g., client-only or connector-only systems).

**Configuration:**

```bash
# In ${ORADBA_PREFIX}/etc/oradba_customer.conf
export ORADBA_SHOW_DUMMY_ENTRIES=false
```

**Behavior:**

```bash
# With ORADBA_SHOW_DUMMY_ENTRIES=true (default)
Oracle Homes
---------------------------------------------------------------------------------
...
# With ORADBA_SHOW_DUMMY_ENTRIES=false
Oracle Homes
---------------------------------------------------------------------------------
Data Safe       : dsconha1     available   /appl/oracle/product/exacc-wob-vwg-ha1
```
...
- ✅ Set to false to hide dummy entries
- ✅ Documented in installer comments
- ✅ Useful for non-production environments

**Installation Comments:**

```bash
# Dummy entry for pre-Oracle environment
# To hide this entry from oraup status display:
#   1. Set ORADBA_SHOW_DUMMY_ENTRIES=false in oradba_customer.conf, OR
#   2. Remove this entry once Oracle is installed
dummy:/appl/oracle/product/dummy:N
```

### Oracle Homes Duplicate Prevention

**Feature:** Prevent duplicate entries in oradba_homes.conf

Automatically detects and prevents duplicate Oracle Home registrations by NAME or PATH.

**Duplicate Detection:**

```bash
$ oradba_homes.sh add --name dsconha1 --path /appl/oracle/product/ds1 --type datasafe
[ERROR] Oracle Home 'dsconha1' already exists
[INFO] Use 'oradba_homes.sh remove dsconha1' to remove it first

$ oradba_homes.sh add --name dsconha2 --path /appl/oracle/product/ds1 --type datasafe
[ERROR] Path '/appl/oracle/product/ds1' is already registered as 'dsconha1'
[INFO] Use 'oradba_homes.sh remove dsconha1' to remove it first
```
...
$ oradba_homes.sh dedupe

Analyzing oradba_homes.conf for duplicates...
Found 5 total entries

Duplicate NAME detected: dsconha1
  Keeping: dsconha1:/appl/oracle/product/exacc-wob-vwg-ha1:datasafe:...
  Removing: dsconha1:/appl/oracle/product/exacc-wob-vwg-ha2:datasafe:...

Removed 1 duplicate entries
Kept 4 unique entries
Backup saved to: /appl/oracle/local/oradba/etc/oradba_homes.conf.backup.20260115_220351
```

**Key Features:**

- ✅ Checks NAME field for duplicates
- ✅ Checks PATH field for duplicates
- ✅ Clear error messages with existing entry info
- ✅ New `dedupe` command for cleanup
...
- ✅ Checks directory existence before PATH addition
- ✅ Prevents warning messages about missing directories
- ✅ Cleaner PATH with only valid entries
...
- ✅ Parser now matches actual oradba_homes.conf format
- ✅ Backward compatibility with old field names
- ✅ Search by NAME, ALIAS, or PATH
- ✅ Consistent field positions across all functions
...
- Version shown as 'N/A' for DataSafe, OUD, WebLogic, OMS, EM Agent

### PATH Duplication Issues

- Fixed custom variables (JAVA_HOME, MAVEN_HOME) being added multiple times
- Final PATH deduplication now runs after all config files loaded
- Prevents accumulation of duplicate paths across environment switches
- Consolidated `deduplicate_path()` and `deduplicate_sqlpath()` to use `oradba_dedupe_path()`
- Single source of truth for deduplication logic
- Includes fallback for standalone use

### Oracle Homes Duplicate Issues

- Fixed DataSafe homes appearing twice in oraup.sh display
...
- New `dedupe` command to clean existing duplicates from config file
- Creates backup before modification
...
- Dummy entries now respect ORADBA_SHOW_DUMMY_ENTRIES configuration
...
- Works correctly in both installed and repository contexts

### Product Type Issues

- Fixed 'iclient' validation in oradba_homes.sh (was rejecting valid type)
- Added 'iclient' to valid product types in both validation locations
