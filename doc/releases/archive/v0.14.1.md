...
- `mnt_reg.rcv`: Changed `SET SNAPSHOT CONTROLFILE NAME` → `CONFIGURE SNAPSHOT CONTROLFILE NAME`
- `src/doc/09-rman-scripts.md`: Updated examples:
  - `SET BACKUP OPTIMIZATION ON` → `CONFIGURE BACKUP OPTIMIZATION ON`
  - `SET ARCHIVELOG DELETION POLICY` → `CONFIGURE ARCHIVELOG DELETION POLICY`
...
The following fixes were applied after the initial v0.14.1 release:

### Version Detection Enhancement

- **oradba_check.sh**: Made version detection dynamic instead of hardcoded
  - Now reads version from VERSION file at runtime
...
- **test_oradba_rman.bats**: Fixed test environment configuration
  - Added `ORADBA_ORA_ADMIN_SID` variable to tests
  - Ensures config files are found in correct locations

- **test_oradba_version.bats**: Fixed error message pattern matching
  - Updated pattern from `"ERROR: Unknown option"` to `"Unknown option"`
  - Matches actual script output
...
- src/bin/oradba_rman.sh: Extended template support, added --tablespaces/--datafiles/--pdb CLI params
- src/bin/oradba_check.sh: Made version detection dynamic from VERSION file
- src/bin/oradba_version.sh: Added file deduplication in integrity checks
- src/bin/oradba_validate.sh: Added missing file detection and reporting
- src/bin/longops.sh: Fixed output formatting (LINESIZE, column widths)
- src/templates/etc/oradba_rman.conf.example: Added 3 new parameters
...
- src/rcv/mnt_reg.rcv: Fixed SET → CONFIGURE SNAPSHOT CONTROLFILE
- tests/test_oradba_rman.bats: Fixed test environment configuration
- tests/test_oradba_version.bats: Fixed error message pattern
- All 34 src/rcv/*.rcv scripts: Version 0.14.0 → 0.14.1
...
Total: 50 files changed, ~750 insertions, ~200 deletions
```
