...
- **Dummy Database Detection**: Automatically detects dummy SIDs (oratab `:D` flag)
  and shows environment info only—no SQL queries attempted
...
- Added `is_dummy_sid()` function to [oradba_common.sh](https://github.com/oehrlis/oradba/blob/v0.9.4/src/lib/oradba_common.sh)
  - Checks oratab for `:D` flag before attempting connection
  - Prevents unnecessary SQL errors

### Database Functions ([oradba_db_functions.sh](https://github.com/oehrlis/oradba/blob/v0.9.4/src/lib/oradba_db_functions.sh))
