...
- Added split discovery flags for database homes vs all products
- Product discovery now runs even when database entries already exist
- DataSafe auto-discovery now uses sequential `dsconN` naming and migrates legacy names
...
- `oraup.sh` clearly marks dummy entries as "dummy (→REAL_SID)"
...
- `plugin_build_bin_path` → uses v2 wrapper
- `plugin_build_lib_path` → uses v2 wrapper
- `plugin_adjust_environment` → uses v2 wrapper

**oradba_env_validator.sh**:

- `plugin_get_required_binaries` → uses v2 wrapper with NOARGS

**oradba_env_config.sh**:

- `plugin_get_config_section` → uses v2 wrapper with NOARGS

### Bug Fixes

#### Auto-Discovery Flags and Naming

**Problem**: Auto-discovery flags were renamed but templates/tests and discovery paths were inconsistent:

- Auto-discovery of products did not run when database entries existed
- DataSafe connectors kept legacy path-derived names after auto-registration

**Solution**:

- Added `--auto-discover-oratab` and `--auto-discover-products` flags
- Ensured product discovery runs when non-database homes are empty
- Migrated legacy DataSafe names to sequential `dsconN` identifiers
...
- `oraup.sh` now marks dummy entries as "dummy (→REAL_SID)"
...
------------------------------------------------------------------------------------------
alias_db             database         dummy (→ORCL) /u01/app/oracle/product/19c
...
- **Total Tests**: 1452 tests across 37 test files (100% passing)
