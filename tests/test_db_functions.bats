...
setup() {
    ...
    source "${PROJECT_ROOT}/src/lib/db_functions.sh"
}
...
@test "query functions should use pipe separator" {
    ...
    grep -q "||" "${PROJECT_ROOT}/src/lib/db_functions.sh"
}
...
@test "query functions use proper sqlplus SET commands" {
    ...
    grep -q "HEADING OFF" "${PROJECT_ROOT}/src/lib/db_functions.sh"
    grep -q "FEEDBACK OFF" "${PROJECT_ROOT}/src/lib/db_functions.sh"
    grep -q "VERIFY OFF" "${PROJECT_ROOT}/src/lib/db_functions.sh"
}
...
@test "show_database_status uses formatted output" {
    ...
    grep -q "printf" "${PROJECT_ROOT}/src/lib/db_functions.sh"
}
