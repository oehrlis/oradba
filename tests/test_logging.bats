...
@test "deprecation warnings disabled by default" {
    run bash -c "source ${PROJECT_ROOT}/src/lib/common.sh && log_info 'Test' 2>&1"
    [ "$status" -eq 0 ]
    [[ ! "$output" =~ "deprecated" ]]
}

@test "deprecation warnings shown when enabled" {
    run bash -c "export ORADBA_SHOW_DEPRECATION_WARNINGS=true && source ${PROJECT_ROOT}/src/lib/common.sh && log_info 'Test' 2>&1"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "deprecated" ]]
    [[ "$output" =~ "log_info" ]]
    [[ "$output" =~ "log INFO" ]]
